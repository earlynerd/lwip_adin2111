...
adi_eth_Result_e SinglePairEthernet::enableDefaultBehavior()
{
    ...
    if(result == ADI_ETH_SUCCESS)
    {
        ...
        if(result) Serial.println("fail, setting bcast mac");
    }
    ...
    if (result == ADI_ETH_SUCCESS)
    {
        ...
        if(result) Serial.println("fail, setting own mac");
    }
    ...
    if (result == ADI_ETH_SUCCESS)
    {
        ...
        if(result) Serial.println("fail, setting link callback");
    }

    if (result == ADI_ETH_SUCCESS)
    {
        //result = phyDriverEntry.RegisterCallback(hdev->pPhyDevice[1],linkCallback_C_Compatible,  ADI_PHY_EVT_LINK_STAT_CHANGE, (void*)2);
        //result = registerCallback(linkCallback_C_Compatible, ADI_MAC_EVT_LINK_STAT_CHANGE);
        if(result) Serial.println("fail, setting link 2 callback");
    }
    ...
    for (uint32_t i = 0; i < SPE_NUM_BUFS; i++)
    {
        ...
        if(result) Serial.println("fail, submiting rxbuffer");
    }
    ...
    {
        result = enable();
        if(result) Serial.println("fail, while enable");
    }
    if (result == ADI_ETH_SUCCESS)
    {
        result = syncConfig();
        if(result) Serial.println("fail, sync config 2");
    }
...
uint16_t SinglePairEthernet::sendFrame( uint8_t *data, int dataLen)
{
    ...
    if (dataLen > TX_PACKET_SIZE) dataLen = TX_PACKET_SIZE;
    ...
    txBufDesc[descIdx].port = ADIN2111_TX_PORT_1;// Or whatever your port is
    ...
    adi_eth_Result_e res =  submitTxBuffer(ADIN2111_TX_PORT_1, &txBufDesc[descIdx]);
